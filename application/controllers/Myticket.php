...
class Myticket extends CI_Controller
{
	...
	function myticket_detail($id)
	{
		...
		$sql = "SELECT A.status, A.progress, A.tanggal, A.tanggal_solved, A.tanggal_proses, A.tanggal_solved, F.nama AS nama_teknisi, D.nama, C.id_kategori, A.id_ticket, A.problem_summary, A.problem_detail, A.tanggal, A.file, B.nama_sub_kategori, C.nama_kategori, G.nama_bagian_dept, H.nama_dept, I.nama_kondisi, I.id_kondisi, A.status, A.durasi_solved
                FROM ticket A 
                LEFT JOIN sub_kategori B ON B.id_sub_kategori = A.id_sub_kategori
                LEFT JOIN kategori C ON C.id_kategori = B.id_kategori 
                LEFT JOIN karyawan D ON D.nik = A.reported 
